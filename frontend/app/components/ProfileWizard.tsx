...
export default function ProfileWizard({ onComplete, onSkip }: Props) {
  ...
  return (
    <div className="fixed inset-0 z-50 bg-black/60 backdrop-blur-sm flex items-center justify-center p-4">
      <div className="bg-white dark:bg-zinc-900 border border-gray-200 dark:border-zinc-800 rounded-2xl w-full max-w-lg shadow-2xl animate-fade-up">
        {/* Header */}
        ...
        <div className="px-6 py-5">
          <h2 className="text-base font-semibold text-gray-900 dark:text-white mb-1">{current.title}</h2>
          <p className="text-xs text-gray-400 dark:text-zinc-500 mb-5">{current.subtitle}</p>
            ...
            {current.options.map((opt) => (
              <button
                ...
                className={`w-full text-left px-4 py-3 rounded-xl border transition-all ${
                  selected === opt.value
                    ? "border-accent/60 bg-indigo-50 dark:bg-accent/8 text-gray-900 dark:text-white"
                    : "border-gray-200 dark:border-zinc-700 bg-gray-50 dark:bg-zinc-800/60 text-gray-700 dark:text-zinc-300 hover:border-gray-300 dark:hover:border-zinc-600 hover:bg-gray-100 dark:hover:bg-zinc-800"
                }`}
              >
                <div className="flex items-center gap-3">
                  <div
                    className={`w-3.5 h-3.5 rounded-full border-2 shrink-0 transition-colors ${
                      selected === opt.value ? "border-accent bg-accent" : "border-gray-300 dark:border-zinc-600"
                    }`}
                  />
                  <div>
                    <p className="text-sm font-medium">{opt.label}</p>
                    <p className="text-xs text-gray-400 dark:text-zinc-500 mt-0.5">{opt.description}</p>
                  </div>
                </div>
              </button>
            ))}
        ...
        <div className="px-6 pb-6 flex items-center justify-between">
          <button
            ...
            disabled={step === 0}
            className="text-xs text-gray-400 dark:text-zinc-500 hover:text-gray-600 dark:hover:text-zinc-300 disabled:opacity-30 disabled:cursor-not-allowed transition-colors"
          >
            Back
          </button>
          <button
            ...
            className="px-5 py-2 rounded-lg bg-accent text-white text-sm font-semibold disabled:opacity-40 disabled:cursor-not-allowed hover:bg-accent-hover transition-colors"
          >
