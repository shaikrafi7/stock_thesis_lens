...
- [x] **[S1] Remove hardcoded SECRET_KEY default** — `backend/app/core/config.py:11`  
...
- [x] **[S2] Replace base64 share tokens with UUID** — `backend/app/routers/share.py:17-23`  
...
- [x] **[B1] Evaluation history returns oldest-first** — `backend/app/routers/evaluate.py:120`  
...
- [x] **[B2] `recovered` semantics wrong in score delta** — `backend/app/routers/evaluate.py:92`  
...
- [x] **[B3] Streaming endpoints buffer full response before sending** — `backend/app/routers/thesis.py:241`, `portfolio.py:132`  
  `all_events = list(agent.stream(...))` defeats SSE. Replace with `async for event in agent.stream(): yield event`.

- [x] **[T1] Scheduler nightly eval runs without investor profile** — `backend/app/services/scheduler.py`  
...
- [x] **[T2] Thesis dedup doesn't compare against existing DB theses** — `backend/app/agents/thesis_generator.py`  
...
- [x] **[A2] Deduplicate JSON deserialization on Evaluation model** — `evaluate.py:85`, `share.py:73`, `portfolio.py` (4+ sites)  
  `json.loads(raw or "[]")` repeated everywhere. Add `parsed_broken_points` / `parsed_confirmed_points` properties on `Evaluation` model.

- [x] **[A3] Fix N+1 evaluation load on portfolio page** — `backend/app/routers/portfolio.py:57-79`  
...
- [x] **[A4] Separate Frozen vs Critical importance multipliers** — `backend/app/agents/thesis_evaluator.py`  
  Both use 2.0x, conflating "high importance" with "committed conviction." Fix: critical=2.0x, frozen=1.5x.

- [x] **[A5] Move nightly scheduler out of app process** — `backend/app/services/scheduler.py:39-40`  
  APScheduler inside uvicorn = N workers × N nightly eval runs, race conditions, N × API cost. Use external cron or cloud scheduler.

- [x] **[B4] Rename 30d price calc variable** — `backend/app/agents/signal_collector.py:172`  
...
- [x] **[D1] Rewrite `docs/scoring_algorithm.md`**  
...
- [x] **[D2] Update `docs/agents.md` category names**  
...
- [x] **Thesis point hover breakdown**  
...
- [x] **5 Groups × 2 Points default constraint**  
...
- [x] **"Articulate Your Edge" thesis field**  
