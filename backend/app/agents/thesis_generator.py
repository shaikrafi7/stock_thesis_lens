...
def generate_thesis(ticker: str, company_name: str, investor_profile: dict | None = None, existing_statements: list[str] | None = None, max_groups: int = 6, max_per_group: int = 2) -> list[GeneratedThesis]:
    ...
    try:
        ...
        results = _parse_bullets(data, existing_statements, max_groups, max_per_group)
        if results:
            return results

        logger.warning("thesis_generator: OpenAI returned empty bullets for %s, using fallback", ticker)
        return _parse_bullets(FALLBACK_THESIS, existing_statements, max_groups, max_per_group)
